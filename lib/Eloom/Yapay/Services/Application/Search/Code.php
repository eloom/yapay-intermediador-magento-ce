 ...
 */
class Eloom_Yapay_Services_Application_Search_Code {
	...
	public static function search(Eloom_Yapay_Domains_Account_Credentials $credentials, $code) {
		try {
			...
			$response = Eloom_Yapay_Resources_Responsibility::http($http, new Eloom_Yapay_Parsers_Authorization_Search_Code_Request());
			return $response;
	...
	private static function request(Eloom_Yapay_Resources_Connection_Data $connection, $code) {
		return sprintf(
			"%s/%s/?%s", $connection->buildAuthorizationSearchRequestUrl(), $code, $connection->buildCredentialsQuery()
		);
	}
