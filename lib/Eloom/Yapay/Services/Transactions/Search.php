 ...
 */
class Eloom_Yapay_Services_Transactions_Search {
	...
	public static function check(Eloom_Yapay_Domains_AccountCredentials $credentials, $tokenTransaction) {
		try {
			...
			$response = Eloom_Yapay_Resources_Responsibility::http($http, new Eloom_Yapay_Parsers_Transaction_Search_Request());
	...
	private static function request($connection, $token, $tokenTransaction) {
		return $connection->buildTransactionSearchRequestUrl() . "?token_account={$token}&token_transaction={$tokenTransaction}";
	}
