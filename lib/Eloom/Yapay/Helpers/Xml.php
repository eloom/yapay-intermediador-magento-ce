...
class Eloom_Yapay_Helpers_Xml {
	...
	public function __construct($xml) {
		...
		if (!xml_parse($parser, $xml)) {
			throw new Exception(
				"Yapay_Library XML parsing error: (" . xml_get_error_code($parser) .
	...
	public function getResult($node = null) {
		...
		if ($node) {
			if (isset($result[$node])) {
				...
				throw new Exception("Yapay_Library XML parsing error: undefined index [$node]");
			}
		} else {
			return $result;
		}
